...
class App:
    ...
    def __hide_window__(self, hide: bool):
        win32gui.ShowWindow(self.hwnd, Constants.SW_HIDE if hide else Constants.SW_SHOWNA)
        # if hide:
        #   win32gui.ShowWindow(self.hwnd, Constants.SW_HIDE)
        # else:
        #   win32gui.ShowWindow(self.hwnd, Constants.SW_SHOWNA)

    def __refresh_window__(self, width: int, height: int, x: int, y: int):
        self.__set_window_size__(width, height)
        self.__set_window_position__(x, y)
...
""")
    ...
    def __main__(self):
        ...
        while True:
            ...
            game_window_focused = self.__is_game_focused__()

            events = pygame.event.get()
            for event in events:
                if event.type == pygame.QUIT or not game_window_focused:
            ...
            pygame.display.flip()
            prev_game_window_size = game_window_size
            prev_game_window_position = game_window_position
